        ...
        $(function(){
            $('#dg').datagrid({   
                ...
                toolbar: [{
                	...
                	handler: function(){
                		parent.$("#win").window({ //因为win是在query的父页面中，所以这里要先调parent定位到aindex页面
                			...
                			width: 350,
                			heiht: 150,
                			content:'<iframe src="send_task_save.action" frameborder="0" width="100%" height="100%" />'
                		});
                	}
                } , '-' , {
                	iconCls: "icon-edit",
                	...
                	handler: function(){
                		...
                		if(rows.length==0){
                			...
                			parent.$('#win').window({
                				...
                				width: 350 ,
                				height: 250,
                				content: '<iframe src="send_task_update.action" frameborder="0" width="100%" height="100%" />',
                			});
                		}
                	}
                } ,'-' , {
                		...
                		if(rows.length==0){
                			...
                			$.messager.confirm('删除确认','您确定要删除吗？', function(r){
                				if(r){
                					...
                					//2,发送ajax请求
                					$.post('task_deleteByIds.action', {ids:ids}, function(result){
                						if(result == "true"){
                ...
                columns:[[                     
                    ...
                    {field:'context',title:'任务描述',width:100,
	                	formatter: function(value,row,index){
	                        return "<span title=" +　value + ">" + value + "</span>";
                    }
                    },    
                    {field:'importantLevel',title:'优先级',width:100,
                        formatter: function(value,row,index){
                        	var desc = 2;
                        	switch(value)
                        	{
                            ...
                            return "<span title=" +　value + ">" + desc + "</span>";
                    }
                    },    
                    {field:'hardLevel',title:'难度系数',width:100,
                        formatter: function(value,row,index){
                            return "<span title=" +　value + ">" + value + "</span>";
                    }
                    },    
